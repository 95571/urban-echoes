 ...
 * @description 游戏内容 - 扁平化对话节点 (v84.3.0 - [优化] 移除冗余对话节点)
 * @author Gemini (CTO)
 * @version 84.3.0
 */
window.gameData.dialogues = {
    // --- 交通出行相关对话 ---
    "DIALOGUE_TRAVEL_TO_YUHANG": {
        dialogueText: [{ avatar: 'images/bus.png', name: '旅行提示', text: `你要前往余杭镇吗？` }],
        options: [
            {
                text: "出发 (消耗4时间, 0精力)",
    ...
    "DIALOGUE_TRAVEL_TO_HANGCHENG": {
        dialogueText: [{ avatar: 'images/bus.png', name: '旅行提示', text: `你要返回杭城吗？` }],
        options: [
            {
                text: "出发 (消耗4时间, 0精力)",
                cost: { time: 4, energy: 0 },
                actionBlock: [
                    { action: { type: 'log', payload: { text: '你坐上了返回杭城的长途汽车...' } } },
                    { action: { type: 'change_map', payload: { mapId: 'hangcheng', nodeId: 'map_hangcheng_bus_station' } } },
                    { action: { type: 'log', payload: { text: '你回到了熟悉的杭城。', color: 'var(--log-color-primary)' } } }
                ]
            },
            { text: '我再想想' }
        ]
    ...
    // --- 场景交互对话 ---
    "DIALOGUE_DARK_ALLEY": {
        dialogueText: [{ avatar: 'images/player_dialogue.png', text: "巷子很暗，散发着一股奇怪的味道。你感觉继续深入可能会有危险。" }],
        options: [
            { text: "深入探索", actionBlock: [{ action: { type: 'log', payload: { text: '（功能暂未开放）你决定还是不要冒险。' } } }] },
            { text: "离开巷子" }
        ]
    },
    "DIALOGUE_CHEAT_MENU": {
